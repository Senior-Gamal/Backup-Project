...
@section('content')
    <h1 class="text-xl font-semibold mb-4">Add License</h1>
    <form method="POST" action="{{ route('licenses.store') }}" class="bg-white shadow rounded p-6 max-w-lg">
        @csrf
        <div class="mb-4">
            <label class="block text-sm mb-1">Key</label>
            <input type="text" name="key" value="{{ old('key') }}" required class="w-full border rounded p-2">
        </div>
        <div class="mb-4">
            <label class="block text-sm mb-1">Expiry Date</label>
            <input type="date" name="expiry_date" value="{{ old('expiry_date') }}" class="w-full border rounded p-2">
        </div>
        <div class="mb-4">
